    ...
    let rec generate_possibilities l running_results =
      ...
      | [a] ->
        ...
        List.concat [List.map (fun x -> x + a) running_results;
                     ...
                     List.map (fun x -> (int_of_string ((string_of_int x) ^ (string_of_int a)))) running_results]
      | a::b::rest ->
        if running_results = [] then
          generate_possibilities rest [a + b;a * b;(int_of_string ((string_of_int a) ^ (string_of_int b)))]
        else
...
let () =
  ...
  |> List.filter_map (fun x -> if OperatorCheck.check_possible x then (Some x) else None)
